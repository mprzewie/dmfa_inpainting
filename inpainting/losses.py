...
def nll_masked_batch_loss(
    ...
    X, J, P, M, A, D = [t.double() for t in [X, J, P, M, A, D]]
...
def zero_batch_at_mask_indices(
    ...
    return X_bmx_chw, P_bmx, M_bmx_chw, A_bmx_l_chw, D_bmx_chw
...
def _nll_masked_sample_loss_v1(
    ...
    m_masked, d_masked = [torch.index_select(t, 1, mask_inds) for t in [m, d]]
...
def _unvectorized_gather_sample(
    x: torch.Tensor,
    j: torch.Tensor,
    p: torch.Tensor,
    m: torch.Tensor,
    ...
    m_masked, d_masked = [torch.index_select(t, 1, mask_inds) for t in [m, d]]
    return (
        x_masked.unsqueeze(0).repeat([m_masked.shape[0], 1]),
        p,
        m_masked,
        a_masked,
        d_masked,
...
def _nll_masked_ubervectorized_batch_loss_v1(
    ...
    l_s = a_s.bmm(d_s_inv).bmm(
        a_s.transpose(1, 2)
    )  # zamiast macierzy, a * D -1 (elemnt-wise)
    l_s = l_s + torch.diag_embed(torch.ones_like(l_s[:, :, 0]))
    # equations (4) and (6) from https://papers.nips.cc/paper/7826-on-gans-and-gmms.pdf
    covs_inv_woodbury = d_s_inv - d_s_inv.bmm(a_s.transpose(1, 2)).bmm(
        l_s.inverse()
    ).bmm(a_s).bmm(
        d_s_inv
    )  # M.data(?)[:, range(100), range(100)] = d_inv (wektory, nie macierze diagonalne) - M[:, range(100), range(100)]
...
def _r2_masked_sample_loss(
    ...
    m_masked, d_masked = [torch.index_select(t, 1, mask_inds) for t in [m, d]]
    return torch.stack(
        [p_i * ((x_masked - m_i) ** 2).sum() for (p_i, m_i) in zip(p, m_masked)]
    ).sum()
